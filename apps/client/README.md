...
The public-facing web application for **RetrouveCI**, a lost-and-found platform
for Côte d'Ivoire. Users can post listings for lost or found items, order
QR-code stickers that redirect scanners to a contact page, and manage their
account. The UI is entirely in French.
...
Auth is phone-number based and currently **mock-only**. Any credentials except
password `"000000"` succeed. The `AuthContext` (`contexts/auth-context.tsx`)
stores the user's stickers and listings in memory.
