...
Tout test vit dans un dossier `__tests__/` **à côté** du fichier testé, avec le **même nom** + suffixe `.test.ts` (requis pour la découverte Vitest).

```
domains/transactions/use-cases/
├── create-transaction.use-case.ts
└── __tests__/
    └── create-transaction.use-case.test.ts
```

Règle générale : on teste `<dir>/<name>.ts` dans `<dir>/__tests__/<name>.test.ts`. La structure des `__tests__` reflète exactement celle du code source.

- ✅ Un `__tests__/` par dossier de code testé, pas de dossier `test/` central pour les unitaires.
- ✅ Le nom du fichier de test reprend celui de la source (`*.use-case.test.ts`, `*.mapper.test.ts`, `*.schema.test.ts`).
- ❌ Pas de test à la racine de la feature/domaine ni mélangé avec le code de prod.
- ℹ️ Les schémas `zod` du package de contrats partagé sont testés **depuis l'app qui les consomme** (dans le `__tests__/` du domaine correspondant), pas dans le package lui-même.

Chaque app définit son `include` Vitest sur `**/*.test.ts(x)`, ce qui couvre les `__tests__/`.

### ⚠️ Après un déplacement/renommage de dossier

Les chemins passés à `vi.mock('<path>')` sont des **chaînes** : ils sont invisibles au
typecheck et ne sont pas réécrits par les outils de refactoring. Après tout move/rename,
en plus de repointer les imports :

```bash
grep -rn "vi.mock('.*<ancien-chemin>" --include='*.test.ts*'
```

Un `vi.mock` resté sur l'ancien chemin ne mocke plus rien — les tests cassent (ou pire,
passent en touchant la vraie dépendance) alors que le typecheck est vert.
...
| schémas `zod` (contrats)  | Entrées valides acceptées, chaque entrée invalide rejetée (une par règle), défauts/coercitions appliqués.           |
...
describe('CreateTransactionUseCase', () => {
	...
	it('lève TransactionAmountError quand le montant est négatif', async () => {
		...
		await expect(
			useCase.execute({ amount: -1, currency: 'EUR' }),
		).rejects.toThrow('TransactionAmountError')
	})
})
```
