...
- [ ] `*.action.ts` renvoie `ActionResult` — `{ success: true }` ou `{ success: false, errors? }` — via `zodErrorToFieldErrors` et `withApiOperationError`, jamais une forme ad hoc. Toute route portant un formulaire répond ce contrat depuis E7.G. Exception connue : six actions **sans formulaire** d'`apps/admin` (`posts`, `orders`, `users`, `qr/token`, `notifications`, `contact-messages`) renvoient encore `{ ok, error }` — hors périmètre d'E7, qui ne visait que les formulaires.
