...
> the only use-case that checked publication had no caller. Fixed; the lesson
> recorded in `MIGRATION-PLAN-API.md` §4.4 is that an exported use-case with no
> caller is what made both bugs invisible.
